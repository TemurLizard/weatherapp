...
async def get_weather():
    ...
    url = f"http://api.openweathermap.org/data/2.5/weather?q=Sheffield&appid={api_key}"
    ...
    cursor.execute("INSERT OR REPLACE INTO weather (city, pressure, current_time) VALUES (?, ?, ?)",
                   ('Sheffield', pressure, current_time))
    conn.commit()
    conn.close()

    return {"message": "Pressure data for Sheffield stored successfully"}
...
async def get_weather_for_sheffield():
    ...
    conn = sqlite3.connect('weather.db')
    cursor = conn.cursor()
    cursor.execute("SELECT pressure, current_time FROM weather WHERE city=?", ('Sheffield',))
    row = cursor.fetchone()
    conn.close()

    if row is None:
        raise HTTPException(status_code=404, detail="Pressure data for Sheffield not found")
